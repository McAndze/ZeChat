...
import com.gmail.andreasmartinmorch.danandchat.chmarkup.ChatColour;
...
//		this.setAllowedPlayers(players);
		...
		if ((newformat = chNode.getString("formatting")) != null && !(newformat.isEmpty())){
			...
			this.setFormatting(this.plugin.settings.defaultFormatting);
		}
		String newMeFormat;
		if ((newMeFormat = chNode.getString("me-formatting")) != null && !(newMeFormat.isEmpty())){
			this.setMeFormatting(newMeFormat);
		} else {
			this.setMeFormatting(this.plugin.settings.defaultMeFormatting);
	...
	public boolean verifyPassword(Player player, String password){
		if (player.hasPermission("danandchat.channel.nopassword." + this.getName()) || player.hasPermission("danandchat.channel.nopassword.*")){
			return true;
		} else {
