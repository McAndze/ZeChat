...
public class DanAndChatPlayerListener extends PlayerListener {
	...
	public void onChat(PlayerChatEvent event) {
		...
		if (!player.hasPermission(PermissionChecker.prefix
				+ PermissionChecker.canTalk)) {
			;
			event.getPlayer()
					.sendMessage(
							ChatColor.RED
									+ "You are not allowed to chat. At all. You can chat with me though, but I'm quite shy.");
			return;
		}
		...
		if (c != null) {
			...
			event.getPlayer()
					.sendMessage(
							ChatColor.RED
									+ "You're in an invalid channel. Try your luck with \"/ch g\" or \"/ch l\"");
		}
	}
	...
	public void onPlayerJoin(PlayerJoinEvent event) {
		Player player = event.getPlayer();
		if (!plugin.getPermissionChecker().init.contains(player)) {
			plugin.getPermissionChecker().initializePlayer(player);
		}
	}
	...
	public void onPlayerQuit(PlayerQuitEvent event) {
		...
		plugin.getPermissionChecker().init.remove(player);
	}
